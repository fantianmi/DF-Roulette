...
function initEventListeners() {
  ...
  const event = contract.events.RandomNumber({}, (err, res) => {
    if (res.blockNumber > lastBlockEvent) {
      ...
      spinWheelText = "转动轮盘";
      updateHTML(spinWheelText,"spinWheelText")
...
function printBet(hash) {
  ...
  if (hash.hasOwnProperty('status')) {
    p.innerText += (hash.status ? '中奖' : '未中奖');
  }
  div.appendChild(p);
}
...
function spinWheel() {
  ...
  if(document.getElementById("spinWheelText").innerHTML != "转动轮盘"){
    alert("请先等待轮盘转动完毕")
    return;
  }
  //console.log()
  ...
  contract.methods.spinWheel().send({
    ...
    if (err) {
      spinWheelText = "转动轮盘";
      updateHTML(spinWheelText,"spinWheelText")
      return void showError('to soon to play?', err);
